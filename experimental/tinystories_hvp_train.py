...
Executes the optimization loop and Hessian spectral extraction for the 
...
to actively monitor the geometric phase transition in non-discrete semantic spaces.
"""
...
# ==========================================
# 1. Transformer Architecture (Unfused Computational Graph)
# ==========================================
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Standard dot-product attention without hardware-level fused kernels
        # This explicit unrolling is mandatory to derive exact second-order gradients
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, n_heads, d_ff, max_len):
        super().__init__()
        # Pre-LN configuration to explicitly bound hidden state variance
...
class TinyStoriesTransformer(nn.Module):
    def __init__(self, vocab_size, d_model=256, n_heads=8, d_ff=1024, n_layers=4, max_len=64):
        ...
        # Rigorous geometric conditioning via controlled initialization
        self.apply(self._init_weights)

    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            # Xavier uniform to rule out scale invariance artifacts
...
# ==========================================
# 2. HVP Configuration & Spectral Extraction
# ==========================================
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Hardware architecture: {device}")

MAX_LENGTH = 64
BATCH_SIZE = 32  # Strictly bounded to mitigate VRAM bottlenecks during double backprop
EVAL_STEPS = 50
...
def compute_hvp(model, loss, v):
    # Derive exact directional product without explicitly instantiating the dense Hessian
...
def get_dominant_eigenvalue(model, data_loader_iter, num_iterations=10):
    ...
    # Power Iteration numerical method for spectral approximation
    for _ in range(num_iterations):
        ...
        # L2 Normalization to prevent arithmetic overflow
...
# ==========================================
# Decoupled regularizer acting as the topological catalyst
optimizer = AdamW(model.parameters(), lr=1e-3, weight_decay=1.0, betas=(0.9, 0.98))

# Synthetic DataLoader for rapid analytical verification
...
print("Initiating topological migration...")
model.train()
max_steps = 200
...
for step in range(max_steps + 1):
    ...
    if step % EVAL_STEPS == 0:
        print(f"Step {step} | Training Risk (Loss): {loss.item():.4f} | Computing local topology...")
        try:
            lambda_max = get_dominant_eigenvalue(model, val_loader_iter)
            print(f"---> Extracted Spectrum: lambda_max = {lambda_max:.2f}")
        except Exception as e:
            print(f"HVP Computation Error: {e}")
            break

print("Optimization trajectory successfully executed.")
